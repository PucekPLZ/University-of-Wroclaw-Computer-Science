...
(define (op->proc [op : Op]) : (Value Value -> Answer) ;; 
  (type-case Op op
    ...
    [(sub) (op-num-num->proc -)]
    [(mul) (op-num-num->proc *)]
    [(div) (lambda (v1 v2) ;; dostosowanie dzielenia
             (type-case Value v2
               [(numV n2)
                (if (zero? n2)
                    (err 'eval "division by zero")
                    ((op-num-num->proc /) v1 v2))]
               [else
                (err 'eval "type error")]))]
...
(define (eval [e : Exp]) : Answer
  (type-case Exp e
    ...
    [(opE o l r)
     (do ([v-l (eval l)]
          [v-r (eval r)])
       ((op->proc o) v-l v-r))] ;; usuniety return
    [(ifE b l r)
     (do ([v (eval b)])
       (type-case Value v
         [(boolV u)
...
(module+ test
  (test (run `{try (/ 10 0) 42})
        (valueA (numV 42)))
  (test (run `{try 42 10})
        (valueA (numV 42)))
  (test (run `{try (+ 2 3) (error foo "Error")})
        (valueA (numV 5)))
  (test (run `{try (error foo "Error") 10})
        (valueA (numV 10)))
  (test (run `{try (error foo "Error") (+ 2 3)})
        (valueA (numV 5)))
  (test (run `{try (error foo "Error") (error foo "Error1")})
        (errorA 'foo "Error1")))
