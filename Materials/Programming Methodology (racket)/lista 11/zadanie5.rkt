...
(define (remove [x : Symbol] [lst : (Listof Symbol)]) : (Listof Symbol)
...
(define (fv [e : Exp]) : (Listof Symbol)
  (type-case Exp e
    ...
    [(opE x l r)
     (append (fv l) (fv r))]  
    [(ifE b l r)
     (append (fv b) (append (fv l) (fv r)))]  
    [(varE x)
     (list x)]  
    [(letE x e1 e2)
     (append (fv e1) (remove x (fv e2)))]  
    [(lamE x b)
     (remove x (fv b))]  
    [(appE e1 e2)
     (append (fv e1) (fv e2))]))  
...
(module+ test
  (test (fv (parse `{let x 1 {+ x {let x {+ 2 1} {* x 3}}}}))
        '())
  (test (fv (parse `{(if (= x 3) double 4) 9}))
        '(x double))
  (test (fv (parse `{(if (= x 1) y {let z 1 {+ z 1}}) 9}))
        '(x y)))