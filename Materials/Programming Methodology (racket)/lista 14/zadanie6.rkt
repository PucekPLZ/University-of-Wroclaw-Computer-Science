...
(define (helper [f : ('a -> (Listof 'b))] [lst : (Listof 'a)]) : (Listof 'b)
  (foldr append empty (map f lst)))

;; helper aplikuje f do każdego elementu lst i konkatenuje do listy
...
(define (showM [c : (M Value)]) : String
  (foldl
   (lambda [(str : String) (acc : String)]
     (string-append acc (string-append str " ; "))) "" (map value->string c)))
