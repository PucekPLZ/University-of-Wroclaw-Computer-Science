...
(define (bindM [c : (M 'a)] [f : ('a -> (M 'b))]) : (M 'b)
  (let ([new (f (fst c))]) 
    (pair (fst new) (append (snd c) (snd new)))))                  

(define (errorM [l : Symbol] [m : String]) : (M 'a)
  (error l m))

(define (showM [c : (M Value)]) : String
  (string-append "value: " (string-append (value->string (fst c)) (string-append ", output: "
  (foldl
   (lambda [(n : Number) (acc : String)]
     (string-append acc (string-append (to-string n) " "))) "" (snd c))))))
...
(define (eval [e : Exp] [env : Env]) : (M Value)
  (type-case Exp e
    ...
    [(writeE e) ;;
     (bindM (eval e env)
            (lambda (v)
              (pair (voidV) (list (value->numV v)))))]
    [(beginE e1 e2) ;;
     (bindM (eval e1 env)
            (lambda (x)
              (eval e2 env)))]))

(define (value->numV [v : Value]) : Number ;;
  (type-case Value v
    [(numV n) n]
    [else (error 'value->numV "not a number value")]))
