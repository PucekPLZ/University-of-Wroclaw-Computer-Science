...
Udowodnij, że definicje returnM i bindM spełniają wymagane prawa równościowe.
...
(bindM (returnM a) f) ≡ z def. returnM
(bindM a f) ≡ z def. bindM
(f a)

2. (bindM c returnM) ≡ c

(bindM c returnM) ≡ z def. bindM
(returnM c) ≡ z def. returnM
c
...
(bindM (bindM c f) g) ≡ z def. bindM
(bindM (f c) g) ≡ z def. bindM
(g (f c))

b) prawa strona:

(bindM c (λ (a) (bindM (f a) g))) ≡ z def. bindM
((λ (a) (bindM (f a) g)) c) ≡ z def. bindM
(bindM (f c) g) ≡ z def. bindM
(g (f c))
