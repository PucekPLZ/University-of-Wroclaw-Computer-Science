  ...
  (λ (v1 v2)
    (type-case Value v1
      [(numV n1)
       (type-case Value v2
         [(numV n2)
          (numV (f n1 n2))] ;; returnM
         [else
          (error 'prim-op "not a number")])] ;; errorM
      [else
       (error 'prim-op "not a number")]))) ;; errorM
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    [(numE n)
     (numV n)] ;; returnM
    [(opE o e1 e2)
     ((op->proc o) (eval e1 env) (eval e2 env))] ;; bindM
    [(varE x)
     (lookup-env x env)]
    [(lamE x b)
     (funV (λ (v) (eval b (extend-env env x v))))] ;; returnM
    [(appE e0 e1)
     (apply (eval e0 env) (eval e1 env))])) ;; bindM

(define (apply [v0 : Value] [v1 : Value]) : Value
  (type-case Value v0
    [(funV f) (f v1)]
    [else (error 'apply "not a function")])) ;; errorM

(define (run [e : S-Exp]) : String
  (value->string (eval (parse e) mt-env))) ;; showM
