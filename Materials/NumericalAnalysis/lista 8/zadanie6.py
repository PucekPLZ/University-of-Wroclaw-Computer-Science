...
sx = interp1d(tk, x_data, kind='cubic', fill_value="extrapolate")
sy = interp1d(tk, y_data, kind='cubic', fill_value="extrapolate")

# Wybór M
M = 1000

# Obliczenia punktów na łamanej
uk = [x/M for x in range(M)]
lx = sx(uk)
ly = sy(uk)

# Rysowanie łamanej
plt.plot(lx, ly, label='Łamana')
plt.xlabel('X')
plt.ylabel('Y')
plt.legend()
plt.show()